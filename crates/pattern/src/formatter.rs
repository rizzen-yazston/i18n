            ...
            if check_node_type( tree, last, NodeType::NamedGroup ) {
                ...
                for named in named_strings.iter() {
                    ...
                    if patterns.contains_key( identifier_token.string.as_str() ) {
                        return Err(
                            FormatterError::NamedStringIdentifier( identifier_token.string.as_str().to_string() )
                        );
                    }
            ...
            match part {
                ...
                PatternPart::PatternDecimal{
                    ...
                    match value {
                        ...
                        PlaceholderValue::Float( number ) => {
                            ...
                            let mut fixed_decimal = FixedDecimal::try_from_f64(
                        ...
                        _ => return Err( FormatterError::InvalidValue( "PatternDecimal".to_string() ) )
                    }
                },
                PatternPart::PatternDateTime{
                    ...
                    match value {
                        ...
                        _ => return Err( FormatterError::InvalidValue( "PatternDateTime".to_string() ) )
                    }
                },
                    ...
                    match complex {
                        ComplexType::Plural => {
                            ...
                            match value {
                                ...
                                _ => return Err( FormatterError::InvalidValue( "PatternPlural".to_string() ) )
                            }
                        },
                        ComplexType::Ordinal => {
                            ...
                            match value {
                                ...
                                _ => return Err( FormatterError::InvalidValue( "PatternOrdinal".to_string() ) )
                            }
                        },
                        ComplexType::Select => {
                            match value {
                                ...
                                _ => return Err( FormatterError::InvalidValue( "PatternSelect".to_string() ) )
                            }
                        },
                    }
                },
    ...
    fn find_number_sign(
        ...
        // Get the named string, and locate number signs to update the string.
        {
            ...
            let Some( named ) = selectors.get( &category ) else {
                return Err( FormatterError::SelectorsIndexNamed( category, selectors_index ) );
            };
...
fn part_pattern(
    ...
    if keyword_token.string.as_str() == "decimal" {
        ...
        for ( selector, named ) in strings.iter() {
            if !option_selectors.valid_plurals.contains( &selector.as_str() ) {
                return Err( FormatterError::InvalidSelector(
                    selector.as_str().to_string(), "plural".to_string(), placeholder_token.string.as_str().to_string()
                ) );
            }
            if !patterns.contains_key( named ) {
                return Err( FormatterError::SelectorNamed(
...
fn sign_display( sign: &str ) -> Result<SignDisplay, FormatterError> {
    match sign {
        ...
        _ => return Err( FormatterError::InvalidOptionValue(
            sign.to_string(), "sign".to_string(), "decimal".to_string()
        ) )
    }
}
...
fn decimal_grouping_display( group: &str ) -> Result<options::GroupingStrategy, FormatterError> {
    match group {
        ...
        _ => return Err( FormatterError::InvalidOptionValue(
            group.to_string(), "group".to_string(), "decimal".to_string()
        ) )
    }
}
...
fn date_length( len: &str ) -> Result<DateLength, FormatterError> {
    match len {
        ...
        _ => return Err( FormatterError::InvalidOptionValue(
            len.to_string(), "date".to_string(), "date_time".to_string()
        ) )
    }
}
...
fn time_length( len: &str ) -> Result<TimeLength, FormatterError> {
    match len {
        ...
        _ => return Err( FormatterError::InvalidOptionValue(
            len.to_string(), "time".to_string(), "date_time".to_string()
        ) )
    }
}
