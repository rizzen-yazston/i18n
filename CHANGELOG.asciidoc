...
** Updated `.sqlite3`: add missing language strings for `NeverReach` error, and languages strings for new error `NoIcuProvider` (spearated from `NeverReach` error) for easier error distinction.
...
** Added new method to retrieve data on supported languages.
