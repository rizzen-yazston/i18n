...
* Added cargo feature `extend` to select more detailed structs, such as `Locale` instead of `LanguageIdentifier`.

* Added experimental directory for prototype and development sub projects.

** Added new subproject *i18n_renderer* for rendering text for various script types.

=== i18n

* Added to `Cargo.toml` the feature `extend` to specify `Locale` to be used instead of `LanguageIdentifier`.

=== i18n_localiser

* Various files, changed language tag parameters to use `LanguaheTag` struct instead.

* Added to `Cargo.toml` the feature `extend` to specify `Locale` to be used instead of `LanguageIdentifier`.

* Fixed test, examples and documentation to reflect the changes.
