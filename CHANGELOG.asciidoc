...
* Added support for rust's concurrency to all the `i18n_` crates, by using `Arc` and `Mutex` instead `Rc` and `RefCell` when enabling the crate feature `sync`. Where necessary also passes `sync` also to the ICU4X crates, that this project depends on.
...
* Renamed `LStringProvider` to `LocalisationProvider`.

* Renamed `ProviderError` to `LocalisationProviderError`.
...
* Renamed `ProviderSqlite3Error` to `LocalisationProviderSqlite3Error`.
